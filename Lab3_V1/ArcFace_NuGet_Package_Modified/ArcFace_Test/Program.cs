...
class Test
{
    static Task<float[]>[] embeddings;
    static Functions functions = new Functions();

    //...................................СОЗДАНИЕ ВЕКТОРОВ EMBEDDING НА ОСНОВЕ ДВУХ ИЗОБРАЖЕНИЙ
    static void CreateEmbeddings(string img1, string img2)
    {
        Image<Rgb24> face1 = Image.Load<Rgb24>(img1);
        Image<Rgb24> face2 = Image.Load<Rgb24>(img2);

        embeddings = new Task<float[]>[]
        {
            functions.CreateEmbedding(face1),
            functions.CreateEmbedding(face2)
        };
    }
    ...
    static (float[,] distance, float[,] similarity) SeparateSyncTest()
    {
        ...
        for (int i=0; i<2; i++)
            for (int j=0; j<2; j++)
            {
                d_res[i, j] = functions.Distance(embeddings[i], embeddings[j]);
                s_res[i, j] = functions.Similarity(embeddings[i], embeddings[j]);
            }
        return (d_res, s_res);
    }
    ...
    static(float distance, float similarity)[,] SimultaniousSyncTest()
    {
        ...
        for (int i=0; i<2; i++)
            for (int j=0; j<2; j++)
                res[i, j] = functions.Distance_and_Similarity(embeddings[i], embeddings[j]);
        return res;
    }
    ...
    static async Task<(float[,] distance, float[,] similarity)> AsyncTest()
    {
        ...
        var test11 = functions.AsyncDistance(embeddings[0], embeddings[0], GenreateTokenKey());
        var test12 = functions.AsyncDistance(embeddings[0], embeddings[1], GenreateTokenKey());
        var test21 = functions.AsyncDistance(embeddings[1], embeddings[0], GenreateTokenKey());
        var test22 = functions.AsyncDistance(embeddings[1], embeddings[1], GenreateTokenKey());
        ...
        test11 = functions.AsyncSimilarity(embeddings[0], embeddings[0], GenreateTokenKey());
        test12 = functions.AsyncSimilarity(embeddings[0], embeddings[1], GenreateTokenKey());
        test21 = functions.AsyncSimilarity(embeddings[1], embeddings[0], GenreateTokenKey());
        test22 = functions.AsyncSimilarity(embeddings[1], embeddings[1], GenreateTokenKey());
    ...
    static async Task<(float[,] distance, float[,] similarity)> AsyncCancelTest()
    {
        ...
        var test11 = functions.AsyncDistance(embeddings[0], embeddings[0], same_faces_test_token1);
        var test12 = functions.AsyncDistance(embeddings[0], embeddings[1], GenreateTokenKey());
        var test21 = functions.AsyncDistance(embeddings[1], embeddings[0], GenreateTokenKey());
        var test22 = functions.AsyncDistance(embeddings[1], embeddings[1], same_faces_test_token2);
        ...
        test11 = functions.AsyncSimilarity(embeddings[0], embeddings[0], same_faces_test_token1);
        test12 = functions.AsyncSimilarity(embeddings[0], embeddings[1], GenreateTokenKey());
        test21 = functions.AsyncSimilarity(embeddings[1], embeddings[0], GenreateTokenKey());
        test22 = functions.AsyncSimilarity(embeddings[1], embeddings[1], same_faces_test_token2);
    ...
    {
        CreateEmbeddings("face1.png", "face2.png");
